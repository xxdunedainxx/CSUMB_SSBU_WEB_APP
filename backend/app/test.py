...
from test.integation.IntegrationTests import IntegrationTests
...
def integration():
  if TEST_CONF.INTEGRATION_ENABLED():
    LogFactory.MAIN_LOG.info(f"Integation Test Toggle: {TEST_CONF.UNIT_TESTING_ENABLED}")
    IntegrationTests.run_integration_tests()
  else:
    LogFactory.MAIN_LOG.info("INTEGRATION TESTS DISABLED")
