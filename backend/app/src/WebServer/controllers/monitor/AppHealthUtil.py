...
from src.threading.ThreadPool import process_is_alive, WorkerPool
...
class AppHealthStatusUtil:
  ...
  @staticmethod
  def is_external_service(service: str) -> bool:
    return Services.redis == service or Services.db == service

  @staticmethod
  def check_service_pids(service: str) -> bool:
    if AppHealthStatusUtil.is_external_service(service):
      return True
  ...
  def get_status(service: str) -> str:
    ...
    elif service == Services.db:
      return AppHealthStatusUtil.check_db_health()
    # elif service == Services.reactApp:
  ...
  def write_status(service: str, status: str):
    LogFactory.MAIN_LOG.info(f"Service status update: [{service}:{status}]")
  ...
  def check_redis_health():
    if Services.redisService.health_check() == True:
      return AppHealthStatus.HEALTHY
    else:
  ...
  @staticmethod
  def check_db_health():
    if Services.dbQueryFactory.check_health():
      return AppHealthStatus.HEALTHY
    else:
      return AppHealthStatus.UNHEALTHY
